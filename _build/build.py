...
{chr(10).join(tracks)}
  ...
  <section class="sec" id="common">
    ...
    <div class="slot">
      <span class="slot__tag">Слот под контент</span>
      <h3>Сюда встанет сводное решение по трём трекам</h3>
      <p>Место под общий текст: связка UX/UI, уроков и багов в одно решение —
        что делаем, в каком порядке и что это даёт. Пришли текст, и он встанет сюда
        в той же вёрстке, что и остальные разделы.</p>
      <ul>
        <li>Сводная гипотеза по продукту</li>
        <li>Что общего у трёх треков и где они пересекаются</li>
        <li>Порядок внедрения и метрики целиком</li>
      </ul>
    </div>
  </section>
